...
def clone_repo(repo_url, dest_folder):
    ...
    try:
        if os.path.exists(dest_folder):
            ...
            subprocess.run(['git', 'pull'], cwd=dest_folder, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
            with print_lock:
                print(f"{Fore.CYAN}Updated {repo_url} in {dest_folder}{Style.RESET_ALL}")
        else:
            subprocess.run(['git', 'clone', repo_url, dest_folder], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
            with print_lock:
